...
from django.http import JsonResponse
from .models import Servicio  # Importa tu modelo
from django.shortcuts import get_object_or_404
from django.views.decorators.csrf import csrf_exempt  # Si estás teniendo problemas con CSRF (solo para depuración, ¡quitar en producción!)
import json  # Para manejar el JSON en el POST
@csrf_exempt
    ...
    if request.method == 'GET':
        if servicio_id is not None:
            ...
            data = {
                ...
                'imagen_cuadro': request.build_absolute_uri(servicio.imagen_cuadro.url) if servicio.imagen_cuadro else None,
                'imagen_ventana': request.build_absolute_uri(servicio.imagen_ventana.url) if servicio.imagen_ventana else None,
            }
            return JsonResponse(data)
        else:
            ...
            data = [{
                ...
                'mostrar_boton_agendar': s.mostrar_boton_agendar,
                'imagen_cuadro': request.build_absolute_uri(s.imagen_cuadro.url) if s.imagen_cuadro else None,
                'imagen_ventana': request.build_absolute_uri(s.imagen_ventana.url) if s.imagen_ventana else None,
            } for s in servicios]
            return JsonResponse({'servicios': data})  # Devuelve un diccionario con la clave 'servicios'

    elif request.method == 'POST':
        #  Aquí es donde manejas la creación o actualización
        try:
            data = request.POST  # O request.body si es JSON, entonces usa json.loads(request.body)

            if servicio_id:  # Actualizar
                servicio = get_object_or_404(Servicio, id=servicio_id)
                servicio.nombre = data.get('nombre')
                servicio.titulo_ventana = data.get('titulo_ventana')
                servicio.subtitulo_ventana = data.get('subtitulo_ventana')
                servicio.contenido_ventana = data.get('contenido_ventana')
                servicio.mostrar_boton_agendar = data.get('mostrar_boton_agendar') == 'on'  # Convertir 'on' a booleano

                if 'imagen_cuadro' in request.FILES:
                    servicio.imagen_cuadro = request.FILES['imagen_cuadro']
                if 'imagen_ventana' in request.FILES:
                    servicio.imagen_ventana = request.FILES['imagen_ventana']
                ...
                Servicio.objects.create(
                    ...
                    titulo_ventana=data.get('titulo_ventana'),
                    subtitulo_ventana=data.get('subtitulo_ventana'),
                    contenido_ventana=data.get('contenido_ventana'),
                    mostrar_boton_agendar=data.get('mostrar_boton_agendar') == 'on',
                    imagen_cuadro=request.FILES.get('imagen_cuadro'),
                    imagen_ventana=request.FILES.get('imagen_ventana'),
                )
